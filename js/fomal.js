...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 如月雫🥝")[0];
}
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Ksrgszk🥝 %c 你正在访问Ksrgszk🥝の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Ksrgszk🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Ksrgszk🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("04/12/2023 19:18:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("04/12/2023 19:18:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://p.inari.site/guest/23-12/05/656e8d588d668.jpg);
    --darkmode-bg:url(https://p.inari.site/guest/23-12/05/656e8d588d668.jpg);
